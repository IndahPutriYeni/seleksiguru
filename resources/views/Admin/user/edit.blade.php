...
<div class="mb-12 px-4 md:ml-4 mt-8">
  <div class="relative flex flex-col min-w-0 break-words w-full mb-6 shadow-lg rounded bg-white">
    ...
    <div class="block w-full overflow-x-auto p-8">
      <!-- Projects table -->
      <form method="POST" action="{{ route('admin.putUser', $user->id) }}">
        @csrf
        @method('PUT')
        <div class="block my-4">
          <label for="name" class="text-small font-semibold">Nama</label>
          <input type="text" class="border-0 px-3 py-3 placeholder-slate-500 text-slate-600 bg-white rounded text-sm shadow focus:outline-none focus:ring w-full ease-linear transition-all duration-150" name="name" id="name" value="{{ $user->name }}" />
        </div>
        <div class="block my-4">
          <label for="email" class="text-small font-semibold">Email</label>
          <input type="text" class="border-0 px-3 py-3 placeholder-slate-500 text-slate-600 bg-white rounded text-sm shadow focus:outline-none focus:ring w-full ease-linear transition-all duration-150" name="email" id="email" value="{{ $user->email }}" />
        </div>
        <div class="block my-4">
        ...
        <div class="block my-4">
          ...
          <select class="border-0 px-3 py-3 placeholder-slate-500 text-slate-600 bg-white rounded text-sm shadow focus:outline-none focus:ring w-full ease-linear transition-all duration-150" name="jabatan" required>
            <option value="kepala_sekolah" @if ($user->jabatan == 'kepala_sekolah') selected @endif>Kepala Sekolah</option>
            <option value="kepala_yayasan" @if ($user->jabatan == 'kepala_yayasan') selected @endif>Kepala Yayasan</option>
